...
public class PipeAdjustorItem extends Item {
    public PipeAdjustorItem() {
    ...
    public @NotNull InteractionResult useOn(UseOnContext pContext) {
        ...
        if (!compoundTag.contains("Start")) {
            if (state.getBlock() instanceof IPipeConnectableBlock<?>) {
                if (!level.isClientSide) {
                    player.displayClientMessage(Lang.translateDirect("pipe_adjustor.notify.first_node_selected")
        ...
        if (player.isSteppingCarefully()) {
            if (!level.isClientSide) {
                player.displayClientMessage(Lang.translateDirect("pipe_adjustor.notify.selection_cleared").withStyle(ChatFormatting.RED), true);
        ...
        if (state.getBlock() instanceof IPipeConnectableBlock<?>) {
            if (hasConnectionBetween(level, start, pos)) {
                ...
                if (!level.isClientSide) {
                    player.displayClientMessage(Lang.translateDirect(hasEnd ? "pipe_adjustor.notify.pipe_re_selected" : "pipe_adjustor.notify.pipe_selected")
                            .withStyle(ChatFormatting.GREEN), true);
                ...
                if (!level.isClientSide) {
                    player.displayClientMessage(Lang.translateDirect("pipe_adjustor.notify.no_valid_pipe_between_these_nodes").withStyle(ChatFormatting.RED), true);
                    stack.setTag(null);
                }
        ...
        if (compoundTag.contains("End")) {
            if (!(level.getBlockState(start).getBlock() instanceof IPipeConnectableBlock<?> startPipe)) {
                if (!level.isClientSide) {
                    player.displayClientMessage(Lang.translateDirect("pipe_adjustor.notify.start_node_not_available").withStyle(ChatFormatting.RED), true);
                    stack.setTag(null);
                }
                return InteractionResult.FAIL;
            }
            ...
            if (!(level.getBlockState(end).getBlock() instanceof IPipeConnectableBlock<?> endPipe)) {
                if (!level.isClientSide) {
                    player.displayClientMessage(Lang.translateDirect("pipe_adjustor.notify.end_node_not_available").withStyle(ChatFormatting.RED), true);
                    stack.setTag(null);
                }
                return InteractionResult.FAIL;
            }
            if (!hasConnectionBetween(level, start, end)) {
                if (!level.isClientSide) {
                    player.displayClientMessage(Lang.translateDirect("pipe_adjustor.notify.no_valid_pipe_between_selected_nodes").withStyle(ChatFormatting.RED), true);
                    stack.setTag(null);
                }
                return InteractionResult.FAIL;
            }

            if (state.getBlock() instanceof PipeCurveAnchorBlock) {
                BezierConnection bezier = new BezierConnection(Couple.create(start, end), pos);
                if (bezier.getLength() > 64) {
                    if (!level.isClientSide)
                        player.displayClientMessage(Lang.translateDirect("pipe_adjustor.notify.pipe_too_long")
                                .withStyle(ChatFormatting.RED), true);
                    return InteractionResult.FAIL;
                }
                startPipe.adjustPipeConnectionShape(level, start, end, pos);
                endPipe.adjustPipeConnectionShape(level, end, start, pos);
            } else {
                startPipe.adjustPipeConnectionShape(level, start, end, null);
                endPipe.adjustPipeConnectionShape(level, end, start, null);
            }
            if (level.isClientSide)
                return InteractionResult.SUCCESS;
            ...
            player.displayClientMessage(Lang.translateDirect("pipe_adjustor.notify.successfully_adjust")
                    .withStyle(ChatFormatting.GREEN), true);
            return InteractionResult.SUCCESS;
        }
    ...
    public void appendHoverText(@NotNull ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, @NotNull TooltipFlag pIsAdvanced) {
        ...
        pTooltipComponents.add(Lang.translateDirect("tooltips.pipe_adjustor"));
    }
